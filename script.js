...
var lowersCheckbox = document.querySelector("input[name=lowers]");
var uppersCheckbox = document.querySelector("input[name=uppers]");
var numbersCheckbox = document.querySelector("input[name=numbers]");
var specialsCheckbox = document.querySelector("input[name=specials]");
...
function writePassword() {
  ...
  function generatePassword() {
    ...
    function uppers_change(checkbox) {
      //If it is checked.
      if (checkbox.checked) {
    ...
    function numbers_change(checkbox) {
      //If it is checked.
      if (checkbox.checked) {
        alert("Checkbox has been ticked!");
      }
    ...
    function specials_change(checkbox) {
      //If it is checked.
      if (checkbox.checked) {
        alert("Checkbox has been ticked!");
      }
    ...
    if (
      ...
      numbersPrompt === false &&
      specialsPrompt === false
    ) {
      alert(
        "You MUST choose at least one of the character prompts: lower case, upper case, numbers, or special characters."
      );
      passwordString =
        "Hit the Generate Password button and please be sure to chooose 'OK' for at least one of the character prompts for your password: upper case, lower case, numbers, or special characters.";
    ...
    if (lowerCasePrompt === true) {
      ...
      let lowerCaseRandomizer = Math.floor(
        Math.random() * lowerCasePool.length
      );
    ...
    if (upperCasePrompt === true) {
      ...
      //Tried to invoke criteriaInvoked++ iterater here, but elected for long form to avoid strange results.
      criteriaInvoked = criteriaInvoked + 1;
      let upperCaseRandomizer = Math.floor(
        Math.random() * upperCasePool.length
      );
